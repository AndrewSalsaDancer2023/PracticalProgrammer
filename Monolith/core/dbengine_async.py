...
class AsyncDocumentRepository(AbstractDocumentRepository):
    ...
    async def drop_collection(self, db_name: string, coll_name: string) -> bool:
        db = self.client[db_name]
        # collection = db[coll_name]
        res = await db.drop_collection(coll_name)
        return res['ok'] != 0.0

    async def get_documents_number(self, db_name: string, coll_name: string) -> int:
        db = self.client[db_name]
        collection = db[coll_name]
        return await collection.count_documents({})

    async def get_collection_names(self, db_name: string) -> list[string]:
        db = self.client[db_name]
    ...
    async def insert_documents_in_database(self, gen, callback, db_name: string, coll_name: string, doc_limit: int = 100) -> int:
        db = self.client[db_name]
        collection = db[coll_name]
        cur_content = []
        num_doc_writes = 0
        for dmp in gen:
            ...
            if len(cur_content) >= doc_limit:
                res = await collection.insert_many(cur_content)
...
# ]
    async def perform_pipeline_operation(self, db_name: string, coll_name: string, pipeline: Pipeline, result_key: string) -> list[dict]:
        db = self.client[db_name]
        collection = db[coll_name]
    ...
    async def find_document(self, db_name: string, coll_name: string, search_filter: dict, compl_key: string) -> list:
        db = self.client[db_name]
        collection = db[coll_name]
        result = await collection.find_one(search_filter)
        if not result:
            return []
        return result[compl_key]

    async def find_all_documents(self, db_name: string, coll_name: string, search_filter: dict, compl_key: string) -> list[list]:
        db = self.client[db_name]
        collection = db[coll_name]
    ...
    async def modify_all_documents(self, db_name: string, coll_name: string, search_filter: dict, update_filter: dict) -> bool:
        db = self.client[db_name]
        collection = db[coll_name]
        res = await collection.update_many(search_filter, update_filter)

        return res.modified_count > 0

    # https: // realpython.com / python -async-iterators /
    async def perform_pipeline_operation_for_fetch(self, db_name: string, coll_name: string, pipeline: Pipeline):
        db = self.client[db_name]
        collection = db[coll_name]
